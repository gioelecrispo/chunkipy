...
class TestTextChunker(unittest.TestCase):
    ...
    def test_overlapping_less_than_max(self):
        ...
        text = "In this unit test, we are evaluating the overlapping functionality." \
               ...
               "long text that exceeds 100 tokens. By setting the overlap_percent to 0.2, we expect the " \
               "generated chunks to have an overlap of approximately 20%. This will help us verify the effectiveness " \
        ...
        expected_chunks = ['In this unit test, we are evaluating the overlapping functionality. This is '
                           'a feature of the TextChunker class, which is important for a proper context '
                           'keeping. The goal is to ensure that overlapping chunks are generated '
                           'correctly. For this purpose, we have chosen a long text that exceeds 100 '
                           'tokens.',
                           'By setting the overlap_percent to 0.2, we expect the generated chunks to '
                           'have an overlap of approximately 20%. This will help us verify the '
                           'effectiveness of the overlapping feature. The TextChunker class should be '
                           'able to handle this scenario and produce the expected results.',
                           "Let's proceed with running the test and asserting the generated chunks for "
                           'proper overlap.']
        self.assertEqual(chunks, expected_chunks)

    def test_overlapping(self):
        # Initialize TextChunker with overlap_percent = 0.3
        ...
        expected_chunks = ['In this unit test, we are evaluating the overlapping functionality. This is '
                           ...
                           'correctly. For this purpose, we have chosen a long text that exceeds 100 '
                           'tokens.',
                           'For this purpose, we have chosen a long text that exceeds 100 tokens. By '
                           'setting the overlap_percent to 0.3, we expect the generated chunks to have '
                           'an overlap of approximately 30%. This will help us verify the effectiveness '
                           'of the overlapping feature.',
